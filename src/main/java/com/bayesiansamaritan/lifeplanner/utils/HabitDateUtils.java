...
public class HabitDateUtils {
    ...
    public Date getNextDueDate(Date currentDueDate, String scheduleTypeName, String reference, Date currentDate){
        if (scheduleTypeName.equals("daily")){
            Daily daily = dailyRepository.findByReferenceId(reference);
            Date newCurrentDate = getEndOfDay(currentDate);
            Date newDueDate = new Date(newCurrentDate.getTime() +24*60*60*1000*daily.getEvery());
            return newDueDate;
        }
...
//        Weekly every scenario case left when not done on correct day
            ...
            LocalDate localDueDate = LocalDate.parse(new SimpleDateFormat("yyyy-MM-dd").format(currentDueDate));
            LocalDate currentLocalDate = LocalDate.parse(new SimpleDateFormat("yyyy-MM-dd").format(currentDate));
            Boolean match = false;
            while (!match) {
                ...
                for (DayOfWeek dayOfWeek1 : daysOfWeek) {
                    if (dayOfWeek1.name().equals(localDueDate.getDayOfWeek().name())) {
                        if (getEndOfDay(currentDate).compareTo(getEndOfDay(newDueDate))<0){
                            match = true;
                        }
